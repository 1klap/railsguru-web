  ...
  describe "GET /auth/:provider/callback" do
    context "existing user with omni_auth_identity" do
      it "creates no user and signs in" do
        mock_omniauth_provider("github", email: existing.email, uid: existing.omni_auth_identities.first.uid)
    ...
    context "existing user without omni_auth_identity" do
      it "creates no user and signs in" do
        mock_omniauth_provider("github", email: existing_no_identity.email, uid: 45678)
        ...
        expect(response).to redirect_to root_path
        follow_redirect!
        expect(response.body).to include existing_no_identity.email
      end
    end
